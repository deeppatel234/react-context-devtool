export function installHook (target) {
    ...
    const doWorkWithHooks = (hook) => {
        if (hook.__reactContextDevtoolHookType == "useReducer") {
            ...
            if (!debugId) {
                const currentReducer = hook.queue.lastRenderedReducer.bind(
                    hook.queue
                );
    ...
    const debugFiber = (container) => {
        ...
        if (!renderer) {
            console.error('useReducer hook debugger is not working due to some internal issue. please report an issue');
        }
        ...
        if (
            ...
            console.error(
                "onCommitFiberRoot is not available in ReactDOM library"
            );
        }
    };
